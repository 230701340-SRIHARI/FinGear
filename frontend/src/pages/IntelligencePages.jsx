  ...
  return (
    <>
      ...
      <Card style={{ marginTop: '16px' }}>
        ...
        {/* Header Row */}
        <div style={{ display: "grid", gridTemplateColumns: "minmax(220px, 2fr) minmax(130px, 1fr) 110px", gap: "16px", padding: "0 12px 8px", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", borderBottom: "1px solid var(--border-color)", marginBottom: "4px" }}>
          <span>Outcome Dimension</span>
          <span>Diagnostic Performance</span>
          <span style={{ textAlign: "right" }}>Points Earned</span>
        </div>

        <div className="health-components" style={{ display: "grid", gap: "12px" }}>
            ...
            <article key={item.label || item.id} style={{ display: "grid", gridTemplateColumns: "minmax(220px, 2fr) minmax(130px, 1fr) 110px", alignItems: "center", gap: "16px", padding: "12px", background: "rgba(255, 255, 255, 0.02)", borderRadius: "8px", border: "1px solid var(--border-color)" }}>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <Progress value={item.value} tone={item.value >= 75 ? "success" : item.value >= 50 ? "info" : "warning"} style={{ flex: 1 }} />
                  <span style={{ fontSize: "11px", fontWeight: 600, minWidth: "32px", textAlign: "right" }}>{item.value}%</span>
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
                ...
                <span style={{ fontSize: "11px", color: "var(--text-muted)", display: "block" }}>/{item.max_points} pts</span>
              </div>
            </article>
          ))}

          {/* Total Summary Footer */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px", background: "var(--surface-hover)", borderRadius: "8px", marginTop: "4px", border: "1px solid var(--border-color)" }}>
            <div>
              <strong style={{ fontSize: "13px", color: "var(--text-primary)", display: "block" }}>Total Weighted Health Score</strong>
              <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>Sum of 5 outcome dimensions based on real cash flows</span>
            </div>
            <div style={{ textAlign: "right" }}>
              <strong style={{ fontSize: "18px", color: "var(--accent-success)" }}>{health?.score || 0} / 100 pts</strong>
              <span style={{ fontSize: "11px", color: "var(--text-secondary)", display: "block" }}>{health?.grade || ""}</span>
            </div>
          </div>
        </div>
      </Card>
  ...
  return (
    <>
      <PageHeader 
        ...
        subtitle="Projected financial trajectory based on personalized spending habits, surplus cash flow, and compounding yields." 
        actions={
...
export function Reports() {
  ...
  function downloadReportCSV(reportName) {
    const clientName = user?.name || profile?.name || "Client";
    let content = `Report,${reportName}\nGenerated,${new Date().toLocaleDateString()}\nUser,${clientName}\n\n`;
    if (reportName.includes("Health")) {
      content += `Overall Health Score,${health?.score || 0}/100\nGrade,${health?.grade || "N/A"}\nSummary,${health?.why || ""}\n\nDimension,Points Earned,Max Points,Performance Score,Weight,Diagnostic Summary\n`;
      (health?.components || []).forEach(c => {
        const pts = c.contribution ?? (c.value * c.max_points / 100).toFixed(1);
        content += `"${c.label}",${pts},${c.max_points},${c.value}%,${c.weight_pct}%,"${(c.reason || "").replace(/"/g, '""')}"\n`;
      });
      content += `Total,${health?.score || 0},100,${health?.score || 0}%,100%,"Overall Grade: ${health?.grade || ""}"\n`;
          ...
          <Card style={{ 
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border-color)", paddingBottom: "14px", marginBottom: "16px" }}>
              <div>
                ...
                <small className="muted">Generated on {new Date().toLocaleDateString()} for {user?.name || profile?.name || "Client"}</small>
              </div>
              <button 
            ...
            {selectedReport.name.includes("Health") && (
              <div style={{ display: "grid", gap: "16px" }}>
                ...
                <div>
                  <h3 style={{ fontSize: "14px", marginBottom: "8px" }}>5 Core Outcome Dimensions (0–100 Weighted Breakdown)</h3>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                      <thead>
                        <tr style={{ background: "var(--surface-hover)", textAlign: "left", borderBottom: "1px solid var(--border-color)" }}>
                          ...
                          <th style={{ padding: "10px 12px", width: "140px" }}>Performance</th>
                          <th style={{ padding: "10px 12px", textAlign: "right", width: "120px" }}>Points Earned</th>
                          <th style={{ padding: "10px 12px" }}>Diagnostic Summary</th>
                        </tr>
                      </thead>
                      <tbody>
                        {(health?.components || []).map((comp) => (
                          <tr key={comp.label} style={{ borderBottom: "1px solid var(--border-color)" }}>
                            <td style={{ padding: "10px 12px" }}>
                              <strong style={{ display: "block", color: "var(--text-primary)" }}>{comp.label}</strong>
                              <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>Weight: {comp.weight_pct}% · Max {comp.max_points} pts</span>
                            </td>
                            <td style={{ padding: "10px 12px" }}>
                              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                <Progress value={comp.value} tone={comp.value >= 75 ? "success" : comp.value >= 50 ? "info" : "warning"} style={{ flex: 1 }} />
                                <span style={{ fontSize: "11px", fontWeight: 600, minWidth: "32px", textAlign: "right" }}>{comp.value}%</span>
                              </div>
                            </td>
                            <td style={{ padding: "10px 12px", textAlign: "right" }}>
                              <strong style={{ fontSize: "14px", color: comp.value >= 75 ? "var(--accent-success)" : comp.value >= 50 ? "var(--text-primary)" : "var(--accent-warning)" }}>
                                {comp.contribution ?? comp.value}
                              </strong>
                              <span style={{ fontSize: "11px", color: "var(--text-muted)" }}> / {comp.max_points}</span>
                            </td>
                      ...
                      <tfoot>
                        <tr style={{ background: "rgba(255, 255, 255, 0.03)", fontWeight: 600 }}>
                          <td style={{ padding: "10px 12px" }} colSpan={2}>
                            <strong>Total Weighted Health Score</strong>
                          </td>
                          <td style={{ padding: "10px 12px", textAlign: "right" }}>
                            <span style={{ fontSize: "15px", color: "var(--accent-success)" }}>
                              {health?.score || 0} / 100 pts
                            </span>
                          </td>
                          <td style={{ padding: "10px 12px", fontSize: "12px", color: "var(--text-muted)" }}>
                            Evaluated against Tier {health?.tier_info?.tier || 3} benchmarks
                          </td>
                        </tr>
                      </tfoot>
                    </table>
