import { Activity, ArrowRight, Calculator, Copy, History, RotateCcw, Save, Sparkles, Trash2 } from "lucide-react";
...
const scenarioTypes = ["Salary Change", "Expense Change", "New Loan", "Vehicle Purchase", "Home Purchase", "Increase SIP", "Reduce Spending", "Change Investment Return", "Change Retirement Age", "Custom Scenario"];
      ...
      <section className="simulator-grid">
        <Card>
          ...
          <form onSubmit={submit} className="form-grid">
            ...
            <Field label="Scenario type"><select value={scenario.scenario_type} onChange={(e) => setScenario({ ...scenario, scenario_type: e.target.value })}>{scenarioTypes.map((item) => <option key={item}>{item}</option>)}</select></Field>
            <Field label="Income change"><input type="number" value={scenario.income_change} onChange={(e) => setScenario({ ...scenario, income_change: Number(e.target.value) })} /></Field>
            <Field label="Expense change"><input type="number" value={scenario.expense_change} onChange={(e) => setScenario({ ...scenario, expense_change: Number(e.target.value) })} /></Field>
            <Field label="Extra investment"><input type="number" value={scenario.extra_monthly_investment} onChange={(e) => setScenario({ ...scenario, extra_monthly_investment: Number(e.target.value) })} /></Field>
            <Field label="New EMI"><input type="number" value={scenario.new_monthly_loan_payment} onChange={(e) => setScenario({ ...scenario, new_monthly_loan_payment: Number(e.target.value) })} /></Field>
...
export function ScenarioHistory() {
  const { scenarioHistory } = useFinance();
  return (
    <>
        ...
        <div className="data-table">
          {scenarioHistory.history?.map((item) => (
            <article key={item.id}>
              ...
              <div className="row-actions"><Button variant="ghost"><Copy size={15} /> Duplicate</Button><Button variant="ghost"><Trash2 size={15} /> Delete</Button></div>
            </article>
          ))}
        </div>
      </Card>
